/**
 * Created by Yoichi Masumoto on 2015/02/28.
 */

angular.module('utils').factory('convToHankakuKana', function(){
  var FHConvert = {
    ConvPattern: {
  ...
  return function(s) {
    return FHConvert.ftoh(FHConvert.fkktohkk(FHConvert.hgtokk(s)))
        .replace('ｰ','-').replace('ｬ','ﾔ').replace('ｭ','ﾕ').replace('ｮ','ﾖ').replace('ｯ','ﾂ').replace('　', ' ').toUpperCase();
  };
});
